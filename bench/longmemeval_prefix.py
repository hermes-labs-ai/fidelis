"""
Test: nomic-embed-text with proper search_query/search_document prefixes.

nomic-embed-text was trained with task-specific prefixes:
  - "search_query: " for queries
  - "search_document: " for corpus documents

The baseline does NOT use these. This test measures the impact.

Based on longmemeval_retrieval.py (fixed baseline).
"""
...
# ---------------------------------------------------------------------------
# Retrieval: sub-query decomposition + cosine rerank (with prefixes)
# ---------------------------------------------------------------------------
...
def main():
    ...
    print(f"[prefix] Running on {len(data)} questions with search_query/search_document prefixes")
    ...
    print(f"\n{'='*70}")
    print(f"  PREFIX: cogito recall_b on LongMemEval_{args.split.upper()}  —  {n} questions")
    print(f"{'='*70}\n")
    ...
    out = {
        ...
        "date": "2026-04-15",
        "test": "nomic_prefix_fix",
        "engine": "recall_b + search_query/search_document prefixes",
        "embed_model": "nomic-embed-text (Ollama)",
        "changes": "Added 'search_query: ' prefix to queries, 'search_document: ' prefix to corpus. 2000-char truncation.",
    ...
    out_path = Path(__file__).parent / "results-prefix-2026-04-15.json"
