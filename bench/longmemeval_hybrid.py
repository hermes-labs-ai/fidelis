"""
Test: HYBRID (BM25 + dense) retrieval benchmark.

Runs BM25 and nomic-embed-text cosine search in parallel for each sub-query,
then fuses all ranked lists via RRF. BM25 index is built per-question.

Based on longmemeval_prefix.py (nomic search_query/search_document prefixes,
2000-char truncation, 8 retries capped at 8s).
"""
...
def retrieve_sessions(
    ...
    k_bm25 = min(20, len(corpus_texts))
...
def main():
    ...
    print(f"[hybrid] HYBRID (BM25 + dense) — {len(data)} questions")
    ...
    for qi, entry in enumerate(data):
        ...
        # Build per-question BM25 index
        corpus_tokens = bm25s.tokenize(corpus_texts)
        bm25_index = bm25s.BM25()
        bm25_index.index(corpus_tokens)

        # Pre-embed corpus with search_document: prefix
        all_vecs = []
        for i in range(0, len(corpus_texts), 100):
            ...
            t0 = time.time()
            rankings = retrieve_sessions(question, corpus_texts, all_vecs, bm25_index)
            ...
            if (qi + 1) % 10 == 0 or qi == 0:
                ...
                print(f"  [{qi+1}/{len(data)}] R@1={running_r1:.1%} R@5={running_r5:.1%}  last=R@1={'✅' if r1>0 else '❌'}/R@5={status}  {elapsed:.1f}s  q={question[:50]}")
            continue
        continue

    n = len(metrics[5]["recall_any"])
    print(f"\n{'='*70}")
    print(f"  HYBRID (BM25 + dense): cogito on LongMemEval_{args.split.upper()}  —  {n} questions")
    print(f"{'='*70}\n")
    ...
    out = {
        ...
        "date": "2026-04-15",
        "test": "bm25_hybrid",
        "engine": "BM25 (bm25s) + nomic-embed-text cosine, RRF fusion",
        "embed_model": "nomic-embed-text (Ollama)",
        "changes": (
            "BM25 (bm25s) runs in parallel with dense cosine per sub-query. "
            "All ranked lists (both types) fed into RRF merge. "
            "BM25 index built per-question. "
            "search_query/search_document prefixes retained. "
    ...
    out_path = Path(__file__).parent / "results-hybrid-2026-04-15.json"
