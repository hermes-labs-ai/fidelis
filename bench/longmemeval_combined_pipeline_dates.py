...
def main():
    ...
    for qi, entry in enumerate(data):
        ...
        # --- Build chunk corpus (with date injection) ---
        haystack_dates = entry.get("haystack_dates", [])
        sid_to_date = {}
        for sid, date in zip(entry["haystack_session_ids"], haystack_dates):
            # Extract just YYYY/MM/DD from "2023/05/20 (Sat) 02:21"
            sid_to_date[sid] = date.split(" ")[0] if date else ""

        all_chunks: list[tuple[str, str]] = []
        for sid, session in zip(entry["haystack_session_ids"], entry["haystack_sessions"]):
            raw_chunks = chunk_session(session, sid)
            date_prefix = sid_to_date.get(sid, "")
            # Prepend date to each chunk text
            for chunk_text, chunk_sid in raw_chunks:
                if date_prefix:
                    all_chunks.append((f"{date_prefix}: {chunk_text}", chunk_sid))
                else:
                    all_chunks.append((chunk_text, chunk_sid))
        ...
        if args.no_filter:
            ...
            if route == "skip":
                ...
                # Default: keep S1 (no gap threshold — LLM hurts on unclassified queries)
                final_ranking = ranked_sessions
    ...
    out_path = Path(__file__).parent / "results-combined-pipeline-2026-04-15.json"
