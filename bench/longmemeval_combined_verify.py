...
def main():
    ...
    for qi, entry in enumerate(data):
        ...
        # --- Stage 2: LLM filter (rerank top-5) ---
        if args.no_filter:
            ...
            t1 = time.time()
            reranked_top, method = llm_verify(question, top_sessions)
            filter_elapsed = time.time() - t1
            total_filter_time += filter_elapsed

            # Build final ranking: reranked top + rest unchanged
            final_ranking = reranked_top + ranked_sessions[top_k:]

        filter_methods[method] = filter_methods.get(method, 0) + 1
        ...
        if (qi + 1) % 10 == 0 or qi == 0:
            ...
            print(
                ...
                f"  filter={filt_avg:.0f}ms  method={method}"
            )
    ...
    out_path = Path(__file__).parent / "results-combined-verify-2026-04-15.json"
